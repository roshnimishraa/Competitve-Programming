...
Build: O(n) 
Update/Query: O(logn) per operation 
...
long long querySegmentTree(vector<long long>& tree, int node, int start, int end, int l, int r) {
    ...
    long long leftSum = querySegmentTree(tree, leftChild, start, mid, l, r);
    long long rightSum = querySegmentTree(tree, rightChild, mid + 1, end, l, r);
